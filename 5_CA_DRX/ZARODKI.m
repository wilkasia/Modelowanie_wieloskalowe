...
% Last Modified by GUIDE v2.5 05-Jun-2019 12:08:16
...
handles.ENERGIA = zeros(handles.len, handles.len);
handles.DYSLOKACJE = zeros(handles.len, handles.len);
...
for i=1:handles.len
    for j=1:handles.len
        ...
        handles.komorka(i,j).drx = false;
        handles.komorka(i,j).drx_before = false;
    end
end
...
% --- Executes on button press in zarodkowanie.
function zarodkowanie_Callback(hObject, eventdata, handles)
% hObject    handle to zarodkowanie (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

GRID = handles.GRID;
ENERGIA = handles.ENERGIA;
A = 86710969050178.5;
B = 9.41268203527779;
...
%delta_ro = zeros(1,200);
%delta_ro(1)=0;
%poprzedni = 0;
dyslokacje = zeros(handles.len,handles.len);

for i=1:n
    ro(i) = A/B + (1-A/B)*exp(-B*i*t);
end

for t=1:delta_t:n-1
    delta_ro = ro(t+1) - ro(t);
    ro_sr = delta_ro / (handles.len*handles.len);
    dyslokacje(t) = dyslokacje(t) + x/100 * ro_sr;
    
    delta_ro = (100 - x)/100 * delta_ro;
    while delta_ro > 0
        d = floor(randi(100)+1000000000);
        ...
        delta_ro = delta_ro - d;
        ind = floor(randi(handles.len*handles.len));
 
       dyslokacje(ind) = dyslokacje(ind) + d;
    end
end

ro_c = 4.21*exp(12);

index1=(1:(handles.len*handles.len));
index1 = index1(randperm(length(index1)));

%tyle bedzie rozrzuconych zarodków
liczba_zarodkow = length(dyslokacje(dyslokacje > ro_c));
%troche z tego pojdzie w srodek ziarna
srodek = floor(0.02 * liczba_zarodkow);
%reszta na granice
granice = liczba_zarodkow - srodek;

for i=1:handles.len*handles.len
   if dyslokacje(index1(i)) > ro_c && ENERGIA(index1(i)) > 0
        handles.komorka(i).drx = true;
        handles.komorka(i).drx_before = true;
        dyslokacje(index1(i)) = 0;
        GRID(index1(i)) = handles.counter+1;
        granice = granice - 1;
   end
   if granice == 0
       break;
...
handles.GRID = GRID;
handles.DYSLOKACJE = dyslokacje;

handles.cmap(end,:) = [0 0 0];
cla
pcolor(GRID);
colormap(handles.cmap);
set(gca, 'CLim', [0 handles.counter + 1]);
axis ij
axis off
axis image
...
% --- Executes on button press in rozrost.
function rozrost_Callback(hObject, eventdata, handles)
% hObject    handle to rozrost (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
GRID = handles.GRID;
DYSLOKACJE = handles.DYSLOKACJE;

Grains = handles.komorka;
Grains_2 = Grains;
    ...
    for i=1:handles.len
        for j=1:handles.len
           isOneNeighbourRecrystallizedBefore = false;
            ...
            if Grains(min([j+1,size(GRID,1)]),i).drx_before %góra G
                isOneNeighbourRecrystallizedBefore = true; 
            elseif Grains(max([j-1,1]),i).drx_before %dó³ D
                isOneNeighbourRecrystallizedBefore = true;  
            elseif Grains(j,min([i+1,size(GRID,2)])).drx_before %prawo P
                isOneNeighbourRecrystallizedBefore = true;  
            elseif Grains(j,max([i-1,1])).drx_before %lewo L
                isOneNeighbourRecrystallizedBefore = true; 
            end
            
            if isOneNeighbourRecrystallizedBefore == false
                continue;
            end
                                 
            if isOneNeighbourRecrystallizedBefore
                if DYSLOKACJE(i,j) > DYSLOKACJE(min([j+1,size(GRID,1)]),i) && DYSLOKACJE(i,j) > DYSLOKACJE(max([j-1,1]),i)...
                    && DYSLOKACJE(i,j) > DYSLOKACJE(j,min([i+1,size(GRID,2)])) && DYSLOKACJE(i,j) >DYSLOKACJE(j,max([i-1,1]))
                Grains_2(i, j).drx = true;
                Grains_2(i, j).drx_before = true;
                DYSLOKACJE(i,j) = 0;
                GRID(i, j) = handles.counter+1;
                end
            end
